# Geant4 Learning Home

## Current Learning State

See `Current_Learning_State.md` and canonical Git file `LEARNING_STATE.md`.

## Concept Map

See `Geant4_Concept_Map.md` and canonical Git file `GEANT4_PHYSICS_KNOWLEDGE_MAP.md`.

## Learning Log

See `Learning_Log.md`; complete chronology remains in `LEARNING_PROGRESS.md`.

## Environment / Commands

Do not duplicate environment details in Notion. Use canonical Git files:

- `ENVIRONMENT.md`
- `PROJECT_MAP.md`
- `GEANT4_LEARNING_PROTOCOL.md`

## Study rule

Program completion is not mastery. Record implementation, verification, and learner understanding separately.

